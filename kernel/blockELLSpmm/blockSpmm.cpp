...
void create_buffer(int A_num_rows, int A_num_cols, int A_ell_blocksize, int A_ell_cols, int *dA_columns, __half *dA_values,
    ...
    // Create sparse matrix A in BlockELL format
    cusparseCreateBlockedEll(&matA, A_num_rows, A_num_cols, A_ell_blocksize,
...
void spmm_blockELL(int A_num_rows, int A_num_cols, int A_ell_blocksize, int A_ell_cols, int *dA_columns, __half *dA_values,
             int B_num_cols, int ldb, int ldc, __half *dB, __half *dC, float alpha, float beta) {
    // spmm: dA, dB, dC must be on GPUs
    // ldb, ldc: leading dimension of Matrix B and C
    // CUSPARSE APIs
...
void create_cuSparse_Buffer(
  torch::Tensor x, torch::Tensor W_values, torch::Tensor W_indices, int W_rows, int W_cols, int ellBlocksize, int ellCols, torch::Tensor res
) {
  if (dBuffer == NULL)
  {
    cusparseCreate(&handle);
    ...
    auto W_data_half = W_values.data_ptr<at::Half>();
    auto W_data_cuda_half = reinterpret_cast<__half*>(W_data_half);

    create_buffer(
      W_rows, W_cols, ellBlocksize, ellCols, W_indices.data_ptr<int>(), W_data_cuda_half, 
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("blockELLSpmm_cuSparse", &blockELLSpmm_cuSparse, "Block ELL Spmm (cuSparse)");
